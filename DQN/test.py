...
def evaluate_load_balance():
    ...
    for idx, test_state in enumerate(test_states):
        ...
        vm_load = list(test_state[1:1 + NUM_TASK_TYPES * NUM_VMS_PER_TYPE[int(test_state[0])]])
        vm_load[int(test_state[0]) * NUM_VMS_PER_TYPE[int(test_state[0])] + best_action] += 1
        print(f"选择完对应动作后的虚拟机负载: {vm_load}")
        # 分别输出每个任务类型下的虚拟机负载
        for t in range(NUM_TASK_TYPES):
            start = t * NUM_VMS_PER_TYPE[int(test_state[0])]
            end = start + NUM_VMS_PER_TYPE[int(test_state[0])]
