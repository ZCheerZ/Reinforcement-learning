...
def evaluate_with_true_tasks(agent, episodes=10):
    """
    ...
    for ep in range(episodes):
        env_q.reset()
        vm_load, entity_loads, overload_flag, overload_vms  = env_q.step_batch(all_task_types[ep], agent)
        # 记录每种类型虚拟机的负载方差
    ...
    #         env_r.step(task_type, env_r.prefix_NUM_VMS_PER_TYPE[task_type] + action)
    #         # 下一个任务
    ...
    #         rr_pointer[task_type] = (rr_pointer[task_type] + 1) % NUM_VMS_PER_TYPE[task_type]
    #         env_rr.step(task_type, env_rr.prefix_NUM_VMS_PER_TYPE[task_type] + action)
    #         # 下一个任务
    #         next_task_type = all_task_types[ep][t] if t+1 >= MAX_STEPS else all_task_types[ep][t+1]
...
def evaluate():
    ...
    vm_var_q, entity_var_q= evaluate_with_true_tasks(agent, episodes=10)
