...
def main():
    folder = "data"  # 假设数据文件夹名为data
    num_types = 3 # 假设有3种类型的需求数据
    ...
    max_unsatisfied_per_type = max(len(data[0]),len(data[1]),len(data[2]))  # 每种最多3个不满足（k=0~3）
    total_max_unsatisfied = num_types * max_unsatisfied_per_type  # 3*3=9
    ...
    for i in range(num_types):
        for j in range(max_unsatisfied_per_type + 1):
            if j < len(data[i]):
                a[i][j] = data[i][j]
    ...
    for i in range(num_types-1,-1,-1):
        print(f"应用类型{i}: {now_sla-path[i][t]}", end=" | ")
        now_sla = path[i][t]
        t = path[i][t]
